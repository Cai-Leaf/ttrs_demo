...
class CourseDataManager:
    ...
    def load_user_course_list(self):
        if not self.__user_course_list:
            ...
            for uid, course_list in tmp_data.itertuples(index=False):
                self.__user_course_list.append((uid, set([int(iid) for iid in course_list.split('-')])))
    ...
    def load_user_info(self):
        if self.__user_info is None:
            ...
            for uid, projectid, age, gender, schoolstagecode,  subjectcodein in data[contain_name].itertuples(index=False):
                ...
                self.__user_projectid[uid] = projectid
    ...
    def get_elective_course_pre_data(self):
        ...
        for uid, course_list in self.__user_course_list:
            if uid in uid_list:
                elective_course_pre_data.append((uid, course_list-filter_list[uid]))
    ...
    def get_open_course_pre_data(self):
        ...
        if self.__user_info is None:
            self.load_user_info()
        # 构造选课包
        class_course = defaultdict(lambda: {'subject': defaultdict(set), 'content': set()})
        for uid, iid, _ in self.__user_course_score[['userid', 'courseid', 'score']].itertuples(index=False):
            if uid in self.__user_info:
                user_msg = self.__user_info[uid].split('-')[0].split('*')
                ssc = user_msg[0]
                sc = user_msg[1]
                class_course[ssc]['content'].add(iid)
                class_course[ssc]['subject'][sc].add(iid)
        # 根据用户有过选课及浏览记录的课程构造过滤表
        filter_list = self.make_course_filter()
        ...
        for uid in set(self.__user_course_score['userid'].values):
            if uid in self.__user_info:
                ...
                ssc = user_msg[0]
                sc = user_msg[1]
                candidate_list = class_course[ssc]['subject'][sc]
                if len(candidate_list) < RECOMMEND_COURSE_NUM:
                    candidate_list = class_course[ssc]['content']
                open_course_pre_data.append((uid, list(candidate_list - filter_list[uid])))
    ...
    def save_to_db(self, data, data_type):
        ...
        for uid, course_socre_list in data:
            if len(course_socre_list) > 0:
                ...
                ssc = tmp_ssc_sc[0]
                sc = tmp_ssc_sc[1]
                projectid = self.__user_projectid[uid]
