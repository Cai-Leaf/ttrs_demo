...
class ResourceShareDataManager:
    ...
    def load_user_item(self):
        if self.user_item is None:
            self.user_item = db_data.read_db_to_df(sql="",
    ...
    def load_item_msg_data(self):
        if self.item_msg_data is None:
            self.item_msg_data = db_data.read_db_to_df(sql="",
    ...
    def get_user_near_browse(self):
        ...
        for uid, iid, pid in data.itertuples(index=False):
    ...
    def get_pre_data(self):
        ...
        if self.user_item is None:
            self.load_user_item()
        # 候选课程列表
        pid_class_item_list = self.make_pid_class_item_list()
        # 过滤列表
        ...
        for uid, info in self.user_info.items():
            if uid in uid_list:
                for projectid in self.user_projectid[uid]:
                    user_msg = str(projectid) + '*' + info.split('-')[0]

                    # 构造协同过滤算法使用的候选集
                    candidate_list = pid_class_item_list[user_msg]
                    ...
                    # 将划分后的类别-资源列表加入预测集
                    class_iid_list = []
                    for class_name, iid_list in tmp_class_iid_list:
                        num = int(5 * (class_count[class_name]/all_count))
    ...
    # 构造冷启动用户开放课程预测数据，形如(用户ID-项目ID-学科学段-课程列表)
    def get_cold_pre_data(self):
        ...
        for uid, class_name in self.user_info.items():
            if uid not in uid_list:
                for projectid in self.user_projectid[uid]:
                    user_msg = str(projectid) + '*' + self.user_info[uid].split('-')[0]
                    candidate_list = pid_class_item_list[user_msg]
                    if len(candidate_list) > 0:
                        pre_data.append((uid, projectid, class_name, candidate_list))
        return pre_data

    def make_pid_class_item_list(self):
        if self.user_item is None:
            self.load_user_item()
        if self.user_info is None:
            self.load_user_info()
        pid_class_item_list = defaultdict(set)
        for uid, iid, projectid in self.user_item[['userid', 'itemid', 'projectid']].itertuples(
                index=False):
            if uid in self.user_info:
                user_msg = str(projectid) + '*' + self.user_info[uid].split('-')[0]
                pid_class_item_list[user_msg].add(iid)
        return pid_class_item_list
    ...
    def save_to_db(self, data):
        if self.user_info is None:
            self.load_user_info()
        time = db_data.get_time_from_db(table_name="", colum_name='e_date')
        save_data = []
        uid_list = set()
        ...
        db_data.save_data_to_db(save_data,
                                ...
                                table_name="", is_truncate=True)
        # 将数据保存到已完成项目推荐数据表
        db_data.delete_data_with_userid(list(uid_list), "")
        db_data.save_data_to_db(save_data,
                                contain=['userid', 'resourceid', 'subjectcode', 'schoolstagecode', 'projectid', 'dt',
                                         'recommendation_index'],
                                table_name="", is_truncate=False)
        # 将数据保存到历史推荐数据表
        db_data.save_data_to_db(save_data,
                                contain=['userid', 'resourceid', 'subjectcode', 'schoolstagecode', 'projectid', 'dt',
                                         'recommendation_index'],
